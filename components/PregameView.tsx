...
import { STAT_COLORS, STAT_BG_COLORS, STAT_ICONS, ITEM_REGISTRY } from '../constants';
...
interface PregameProps {
  ...
  onDraftItem: (itemIndex: number) => void;
  onDraftCard: (cardIndex: number) => void;
  onReady: () => void;
}

export const PregameView: React.FC<PregameProps> = ({ gameState, localPlayer, onDraftDie, onDraftItem, onDraftCard, onReady }) => {
  ...
  const handleDraft = () => {
      if (selectedIndex !== null) {
          ...
          else if (!localPlayer.hasDraftedItem) onDraftItem(selectedIndex);
  ...
  const renderContent = () => {
      ...
      // 2. Hero Item Draft
      if (!localPlayer.hasDraftedItem) {
          return (
              <>
                  <div className="text-center text-slate-300 mb-4">
                      Choose a starting Item to aid your quest.
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      {localPlayer.draftItemOptions.map((itemId, idx) => {
                          const item = ITEM_REGISTRY[itemId];
                          return (
                              <div key={itemId} 
                                   onClick={() => setSelectedIndex(idx)}
                                   className={`
                                        bg-slate-800 border-2 p-4 cursor-pointer transition-all flex flex-col items-center gap-2 group
                                        ${selectedIndex === idx ? 'border-yellow-400 ring-2 ring-yellow-400/50' : 'border-slate-600 hover:border-blue-400 hover:bg-slate-700'}
                                   `}>
                                   <div className="w-20 h-20 bg-black border border-slate-700 flex items-center justify-center">
                                        <img src={item.imageUrl} className="w-full h-full [image-rendering:pixelated]" />
                                   </div>
                                   <div className="text-center">
                                        <div className="font-bold text-slate-200">{item.name}</div>
                                        <div className="text-xs text-slate-400 mt-1">{item.description}</div>
                                        {item.bonusStat && <div className={`text-xs mt-1 ${STAT_COLORS[item.bonusStat]}`}>+{item.bonusAmount} {item.bonusStat}</div>}
                                   </div>
                              </div>
                          );
                      })}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {localPlayer.draftDieOptions.map((die, idx) => (
                      <div key={die.id}
                           ...
                           {/* Larger Faces for Mobile */}
                           <div className="w-fit mx-auto grid grid-cols-3 gap-1 md:gap-2 md:w-full">
                                {die.faces.map((f, i) => (
                                    <div key={i} className="flex flex-col items-center gap-0.5">
                                        <div className={`w-12 h-12 md:w-full md:aspect-square rounded border flex items-center justify-center relative bg-slate-900 ${STAT_COLORS[f]}`}>
                                        ...
                                        {/* Increased Font Size */}
                                        <span className="text-[8px] md:text-[6px] uppercase text-slate-400 font-bold tracking-tighter">{f}</span>
                                    </div>
                                ))}
                           </div>
  ...
  return (
    <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-4 gap-6">
      ...
      <div className="w-full max-w-5xl grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          {/* Main Draft Area */}
          <Panel title={isDM ? "Draft Your Trap Deck" : "Forge Your Equipment"} className="min-h-[400px] flex flex-col">
              {localPlayer.isReady ? (
          ...
          <Panel title="Squad Status" className="flex flex-col">
              <div className="flex flex-col gap-2">
                  {gameState.players.map(p => {
                      let status = 'Drafting...';
                      if (p.isReady) status = 'READY';
                      else if (p.role === 'DM') status = 'Drafting Deck...';
                      else if (!p.hasDraftedItem) status = 'Choosing Item...';
                      else status = 'Drafting Dice...';
